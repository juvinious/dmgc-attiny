...
$(DMGC_IPS_ATTINY85): $(BASE_SOURCE)
	$(CC) $(INO_FLAG) ino/$(DMGC_IPS_ATTINY85)/$(DMGC_IPS_ATTINY85).ino $(BASE_SOURCE) $(CFLAGS) $(LDFLAGS) -o bin/$(DMGC_IPS_ATTINY85)

$(DMGC_IPS_ATTINY85_PLUS): $(BASE_SOURCE)
	$(CC) $(INO_FLAG) ino/$(DMGC_IPS_ATTINY85_PLUS)/$(DMGC_IPS_ATTINY85_PLUS).ino $(BASE_SOURCE) $(CFLAGS) $(LDFLAGS) -o bin/$(DMGC_IPS_ATTINY85_PLUS)

$(BREATHING): $(BASE_SOURCE)
	$(CC) $(INO_FLAG) ino/$(BREATHING)/$(BREATHING).ino $(BASE_SOURCE) $(CFLAGS) $(LDFLAGS) -o bin/$(BREATHING)
